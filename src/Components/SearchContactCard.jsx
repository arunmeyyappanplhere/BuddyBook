...
const SearchContactCard = ({
  ...
  return (
    <div
      className="group bg-white w-full rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 flex items-center gap-3 md:gap-4 py-2.5 px-4 cursor-pointer hover:bg-blue-50/70 hover:border-blue-200 border border-transparent"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <h2 className="text-gray-500 text-sm truncate flex items-center gap-1">
          <Phone size={11} className="shrink-0" />
          {contactNumber}
        </h2>
        {contactRelation && (
          <span className="inline-block mt-0.5 text-[11px] font-medium text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full">
            {contactRelation}
          </span>
        )}
      </div>
