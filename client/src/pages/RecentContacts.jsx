...
const RecentContacts = ({ openAddContactModal, setOpenAddContactModal, onEditContact }) => {
  ...
  useEffect(() => {
    const fetchRecents = async () => {
      ...
      try {
        ...
        setContactsError(err.response?.data?.message || "Failed to fetch recent contacts");
      } finally {
        setContactsLoading(false);
      }
    };
   ...
   return (
     <div className="min-h-screen">
       ...
       />
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 md:gap-6">
          ...
          <div className="flex gap-3 items-center lg:self-start shrink-0 md:w-full">
            <div className="text-right">
              ...
              <h2 className="text-gray-600 text-md">{userProfile?.phoneNumber}</h2>
            </div>
            <img
        ...
        {contactsError && (
          <div className="mt-14 flex flex-col items-center gap-4 rounded-4xl border border-red-200 bg-red-50 p-8 text-center">
            <p className="text-red-600 text-lg">Failed to load recent contacts.</p>
            <button className="px-6 py-2 rounded-xl bg-red-500 text-white font-semibold cursor-pointer hover:bg-red-600 transition" onClick={() => window.location.reload()}>
              Retry
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 rounded-4xl text-black px-4 md:px-10">
              {filteredContacts.length > 0 ? (
                ...
                <div className="w-full col-span-full flex flex-col items-center justify-center py-16 text-gray-500">
                  ...
                  <p className="text-xl">No recent contacts match your search.</p>
                  <p className="text-md mt-2">Try adjusting your search text or filters.</p>
                  <button className="mt-6 px-6 py-2.5 rounded-xl bg-blue-500 text-white font-semibold cursor-pointer hover:bg-blue-600 transition" onClick={resetFilters}>
                    Clear all filters
                  </button>
                </div>
              ) : (
                <div className="w-full col-span-full flex flex-col items-center justify-center py-16 text-gray-500">
                  ...
                  <button className="mt-6 px-6 py-2.5 rounded-xl bg-blue-500 text-white font-semibold cursor-pointer hover:bg-blue-600 transition" onClick={() => setOpenAddContactModal(true)}>
                    Add Contact
                  </button>
                </div>
              )}
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 rounded-4xl text-black px-4 md:px-10">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="shadow-md p-5 min-h-36 flex flex-col gap-16 rounded-xl w-full bg-gray-100 animate-pulse" />
              ))}
            </div>
          </>
        )}
